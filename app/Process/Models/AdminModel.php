...
class AdminModel extends DataBase
{
    ...
    {
        $sql= "SELECT `id`, `avatar`, `login`, `email`, `role`,  `token` FROM `admins` ORDER BY `upgrading_status` DESC";
    ...
    public static function getOneUser()
    {
        ...
        $sql= "SELECT `id`, `avatar`, `login`, `email`, `role`, `token` FROM `admins` WHERE `id`= ?";
        $stmt = self::conn()->prepare($sql);
